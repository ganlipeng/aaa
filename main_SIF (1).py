...
from ops.models_SIF import TSN
...
#    print(model)
    ...
    if args.tune_from:
        ...
        for k, v in model_dict.items():
            if 'neg' in k:
   #             print(k)
   #             print(k.replace('_neg', ''))
                sd[k] = sd[k.replace('_neg', '')]
...
def train(train_loader, model, criterion, optimizer, epoch, log, tf_writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        output, output_neg = model(input_var)
        N,_=output.size() #N=batch
        predict_1=softmax(output)
        ...
        SoftlossN=N_Softloss(output_neg,predict_1)
        loss = criterion(output, target_var)+0.3*SoftlossN
